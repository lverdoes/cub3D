...
/*   Updated: 2020/06/11 16:13:51 by lverdoes      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int			check_next(t_map *map, int row, int col, int prev)
{
	...
	if (ft_charsearch(map->map[row][col], "1X")
		...
		return (1);
	if (ft_charsearch(map->map[row][col], "02349"))
	{
		map->map[row][col] = 'X';
...
static int	adjacent(char **map, int i, int j)
{
	if (map[i][j] == 'X')
	{
		if (ft_charsearch(map[i - 1][j], "0239")
		|| ft_charsearch(map[i + 1][j], "0239")
		|| ft_charsearch(map[i][j - 1], "0239")
		|| ft_charsearch(map[i][j + 1], "0239")
		|| ft_charsearch(map[i - 1][j - 1], "0239")
		|| ft_charsearch(map[i + 1][j + 1], "0239")
		|| ft_charsearch(map[i - 1][j + 1], "0239")
		|| ft_charsearch(map[i + 1][j - 1], "0239"))
			return (1);
