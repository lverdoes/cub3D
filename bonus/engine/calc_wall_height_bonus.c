...
/*   Updated: 2020/06/27 13:51:37 by lverdoes      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_screen_column_height(t_vars *vars)
{
	...
	r->col_start += vars->pos.look_up;
	r->col_start += (vars->pos.jump / r->perp_wall_dist);
	if (r->col_start < 0)
		r->col_start = 0;
	else if (r->col_start >= vars->img.height)
		r->col_start = vars->img.height;
	r->col_end = r->col_height * 0.5 + vars->img.height / 2;
	r->col_end += vars->pos.look_up;
	r->col_end += (vars->pos.jump / r->perp_wall_dist);
...
void	check_wall_side(t_vars *vars)
{
	...
	r = &vars->ray;
	p = &vars->pos;
	if (vars->data.map[r->map_y][r->map_x] == '9')
	{
		r->tex_side = 'D';
		r->tex_width = vars->tex[N_TEXTURES - 1].width;
		r->tex_height = vars->tex[N_TEXTURES - 1].height;
	}
	else if (r->side == 0)
	{
