...
/*   Updated: 2020/07/26 08:59:32 by lverdoes      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void			get_color(t_vars *vars, int *color, char *str)
{
	size_t i;

	if (color[0] != -1 || color[1] != -1 || color[2] != -1)
		error_msg(vars, "Multiple input lines for color(s)");
	color[0] = ft_atoi(str + 0);
	i = move_index(vars, str, 0, 1);
	if (str[i] != ',')
		error_msg(vars, "Wrong input for color(s)");
	i++;
	color[1] = ft_atoi(str + i);
	i = move_index(vars, str, i, 1);
	if (str[i] != ',')
		error_msg(vars, "Wrong input for color(s)");
	i++;
	color[2] = ft_atoi(str + i);
	i = move_index(vars, str, i, 1);
	if (str[i] != '\0')
		error_msg(vars, "Wrong input for color(s)");
	if (color[0] > 255 || color[1] > 255 || color[2] > 255 ||
		color[0] < 0 || color[1] < 0 || color[2] < 0)
		error_msg(vars, "Wrong color value(s), must be between 0-255");
}
