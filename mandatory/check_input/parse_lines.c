...
/*   Updated: 2020/06/27 13:33:11 by lverdoes      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_parsed_data(t_vars *vars)
{
	...
	while (i < 3)
	{
		if (d->color_ceiling[i] < 0)
			return (0);
		if (d->color_floor[i] < 0)
			return (0);
		i++;
	}
	return (1);
...
static void	parse_res_tex_col_data(t_vars *vars, char *line, size_t i)
{
	t_data *d;
	...
	if (line[i] == 'R')
		...
		get_color(vars, vars->data.color_floor, line + i + 1);
	else if (line[i] == 'C')
		get_color(vars, vars->data.color_ceiling, line + i + 1);
...
void		parse_lines(t_vars *vars, char *line)
{
	...
	if (line[0] == '\0')
		...
		parse_res_tex_col_data(vars, line, i);
}
